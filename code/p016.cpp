/***********************************************
* Title:        Project Euleur Problem 16: Power Digit Sum
* Author:       Dawson Whipple
* Date:         4/4/24
* Description:  2^15 = 32768 and the sum of its digits is 3 + 2 + 7 + 6 + 8 = 26.
*               What is the sum of the digits of the number 2^1000?
************************************************/
//Author's Notes:
//Did this in the most brute force method possible
//Really need to make the script to turn large digit strings into workable numbers
//probably using int arrays?
//C++
#include <iostream>

using std::cout;

int main(){
    //I did not calculate the value of 2^1000
    //instead I took the value from wolfram alpha:
    
    //manually added addition symbols:
    

    int sum = ;
    cout << "The sume of each digit of 2^1000 is: " << sum;
    return 0;
}
